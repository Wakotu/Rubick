...
public class Generator {
  // public static String main(String metaInfoFile, String alphabetFile, String outApiJSONFile,
  // String outHarnessFile) { 	iMetaInfo meta = iMetaInfo.loadMetaInfoFromFile(metaInfoFile);
  //	iAlphabet al = iAlphabet.createFromFile(alphabetFile);
  //	return generate(meta, al, ???, outApiJSONFile, outHarnessFile);
  // }

  public static String generateMetaAndCode(iMetaInfo meta, iAlphabet al,
      HashMap<Character, iAPIFunc> outApiFuncs, String outHarnessFile) {
    ...
    try {
      ...
      throw new RuntimeException(
          "meet IOexception when trying to dump the generated harness to the file: " + ex);
    }
  }

  public static void generateMetaOnly(
      iMetaInfo meta, iAlphabet al, HashMap<Character, iAPIFunc> outApiFuncs) {
    String unsatisfiedDataDep =
        genHarnessMeta(meta, al, new HashMap<Map.Entry<String, String>, String>(), outApiFuncs);
    if (unsatisfiedDataDep != null)
      /* we need to fix this data dep break before continuing learning */
      throw new RuntimeException("Failed to generate harness: " + unsatisfiedDataDep);
  }
  ...
  private static String genHarnessMeta(iMetaInfo meta, iAlphabet al,
      HashMap<Map.Entry<String, String>, String> outVars,
      HashMap<Character, iAPIFunc> outApiFuncs) {
    Set<Map.Entry<String, String>> fullAccepts = new HashSet<Map.Entry<String, String>>();
    HashMap<Map.Entry<String, String>, Set<String>> accepts =
        new HashMap<Map.Entry<String, String>, Set<String>>();
    ...
    al.getAlias2Letter().forEach((c, l) -> {
      if (l instanceof iConstraintLetter) {
        ...
        System.out.println("letter " + Character.toString(c) + ", cnt: " + l.getContent()
            + ", related: " + related);
    ...
    meta.getDataDeps().forEach((dep) -> {
      Map.Entry<String, String> frmEntry =
          new SimpleEntry<String, String>(dep.frmFuncSig, dep.frmTag);
      ...
      if (dep.frmFuncSig.equals("input"))
        ...
        /* this is a trick used for making this generation process happy when some data deps we
         * currently don't have time to collect */
        /* fake var will never be initialized, therefore it will cause data-dep exception during the
         * runtime */
        var = "fake";
      else
        /* var might be null here, since data deps covers a superset of characters comparing with
         * shrinked alphabet */
        ...
        for (Map.Entry<String, iFuncInMeta> inEntry : fm.getIns().entrySet()) {
          ...
          if ((_acc == null) || (_acc.size() == 0) || (_acc.contains(null))) {
            if (fullAccepts.contains(new SimpleEntry<String, String>(fSig, tag))) {
              ...
              return "Locally: data deps cannot be satisfied in this nfa: func: " + fSig
                  + ", tag: " + tag;
            } else {
  ...
  private static String genHarnessCode(iMetaInfo meta, iAlphabet al,
      HashMap<Map.Entry<String, String>, String> vars, String codeTemplate) {
    ...
    */
    ...
    al.getAlias2Letter().forEach((c, l) -> {
      if (l instanceof iAPILetter) {
        ...
        boolean hasRet =
            fm.hasRet() && (vars.containsKey(new SimpleEntry<String, String>(fSig, "ret")));
        boolean hasItrRet =
            fm.hasItrRet() && (vars.containsKey(new SimpleEntry<String, String>(fSig, "itr")));
        ...
        for (String arg : args) {
          ...
          code = code + FII + ty + " " + arg + " = (" + ty + ") getVarValue(pickInVar(nextCall, \""
              + arg + "\"));\n";
          // debug use
          code = code + FII
              + "if (debug) System.out.println(nextCall + \": getVarValue(\" + pickInVar(nextCall, \""
              + arg + "\") + \") => \" + " + arg + ");\n";
        }
        ...
        if (fm.hasCls()) {
          if (fm.isStatic) {
            // System.out.println("nm is " + nm + ", contains ITERATOR: " +
            // nm.contains("_ITERATOR_"));
            if (nm.contains("_ITERATOR_")) {
              // mock apis for iterating logic
              if (args.size() != 1)
                throw new RuntimeException(
                    "iterable mock api should have 1 and only 1 argument: " + args);
              ...
              if (hasRet) {
                ...
                if (hasItrRet)
                  throw new RuntimeException(
                      "we don't support iterator of iterator currently" + nm);
              ...
              if (args.size() != 1)
                throw new RuntimeException(
                    "iterator mock api should have 1 and only 1 argument: " + args);
              ...
              if (hasItrRet)
                throw new RuntimeException(
                    "loopvar mock api cannot have iterator return value: " + nm);
              ...
              if (hasRet) {
                // has ret & ret value has been used
                code =
                    code + FII + "ret = " + ty + "." + nm + "(" + String.join(",", args) + ");\n";
                code = code + FII + "vars.put(getOutVar(nextCall, \"ret\"), ret);\n";
                if (hasItrRet) {
            ...
            String ty = fm.getIn("cls").ty;
            code = code + FII + ty + " cls = (" + ty
                + ") getVarValue(pickInVar(nextCall, \"cls\"));\n";
            // debug use
            code = code + FII
        ...
        for (String sig : related.keySet()) {
          for (String tag : related.get(sig)) {
            ...
            code = code + CII + ty + " " + var + " = (" + ty + ") getVarValue(getOutVar('"
                + Character.toString(fc) + "', \"ret\"));\n";
          }
        }
